...
save_image(style, './image/style256.png')
...
z = torch.FloatTensor([1.0]).to('cuda') # alpha
output = model(x, y, z)
print(output.size())
...
"AdaInStyleTransfer_alpha.onnx",   # where to save the model (can be a file or file-like object)
export_params=True,        # store the trained parameter weights inside the model file
opset_version=11,          # the ONNX version to export the model to
do_constant_folding=True,  # whether to execute constant folding for optimization
input_names = ['content', 'style', 'alpha'],   # the model's input names
